...
| D8  | Orchestrator does PR creation / git push / Linear status writes (should be agent tools). The dead residue left behind after #76 — the `pr:` (`PRConfig`) and `safety:` (`SafetyConfig`) WORKFLOW.md config blocks (the worker no longer opens/labels PRs and never enforced `safety:`), plus the `push` / `pr_created` task-event vocabulary — was removed under #578; `pr:` / `safety:` are now rejected at load. | §1 boundary, §tools                                       | **P0**   | Closed                  | [#76](https://github.com/xrf9268-hue/aiops-platform/issues/76) / [#578](https://github.com/xrf9268-hue/aiops-platform/issues/578)                                                                       |
